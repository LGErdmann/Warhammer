...
INC_TALENT_ADAPTERS = {
    ...
    "touched by fate": {"kind": "resource", "rule": "+Rank maximum Wrath in the Incursion.", "trigger": "wrath_cap"},
}
...
def init_db():
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS incursion_run(
        ...
        wrath_current INTEGER, wrath_max INTEGER, incursion_statuses TEXT DEFAULT '{}',
...
.inc-wrath-spend{border:1px solid #604d28;background:linear-gradient(180deg,#1d170d,#0e0b07);padding:5px;text-align:center;min-height:44px}.inc-wrath-spend span{display:block;font:8px monospace;letter-spacing:.12em;color:#8b7650}.inc-wrath-spend b{font:700 1.15rem Cinzel,serif;color:#e7c66c}.inc-wrath-spend small{font:8px monospace;color:#756044;margin-left:5px}.inc-wrath-spend+*{}
    ...
    .inc-combat-header,.inc-identity{border:1px solid #5a4523;background:linear-gradient(135deg,#17110b,#0d0a07);padding:16px 18px;margin:8px 0 12px;box-shadow:0 0 24px rgba(0,0,0,.28)}
    .inc-kicker{font-size:.63rem;letter-spacing:.24em;color:var(--gold);text-transform:uppercase;font-family:Cinzel,serif}
    .inc-hero-name{font-family:Cinzel,serif;font-size:1.45rem;color:var(--gold2);letter-spacing:.08em;text-transform:uppercase;margin-top:3px}
    .inc-hero-sub{font-size:.72rem;letter-spacing:.12em;color:var(--bone);opacity:.62;text-transform:uppercase;margin-top:3px}
...
def _inc_start_run(ch):
    ...
    cur = conn.execute(
        ...
        "shock_current,shock_max,wrath_current,wrath_max,xp,xp_earned,incursion_statuses,node,created_at) "
        "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
        (ch["id"], "active", "start", 1, pools["wounds_max"], pools["wounds_max"],
         ...
         json.dumps({}), json.dumps({"type": "choice_start"}), now_iso()))
    conn.commit()
...
def _inc_choose_start(run, ch, choice):
    ...
    if choice == "rest":
        ...
        run = _inc_persist(run["id"], wounds_current=min(run["wounds_max"], run["wounds_current"] + heal),
                            shock_current=run["shock_max"])
        return _inc_persist(run["id"], node={"type": "rest_upgrade"})
    if choice == "shop":
...
def _inc_rest_set_armour(run, ch, armour_key):
    # A loadout choice, not a purchase - free, doesn't touch free_upgrade_used.
...
def _inc_render_choice_start(run, ch):
    ...
    with c1:
        st.markdown("<div class='inc-card'><b>Rest</b><br><span style='font-size:.8rem;opacity:.75'>"
                    "Fully recover Shock and 25% of max Wounds, then a moment to train.</span></div>",
                    unsafe_allow_html=True)
    ...
    for i,e in enumerate(node["enemies"]):
        with cols[i%len(cols)]:
            ...
            st.markdown(
                ...
                f"<div class='inc-vox'><span class='vox-label'>VOX LINK</span><span class='vox-wave'><i></i><i></i><i></i><i></i><i></i><i></i><i></i></span><span class='vox-state'>{('LOCKED' if e['uid'] in selected_targets else 'ACTIVE')}</span></div>"
    ...
    with st.container(border=True):
        if action=="ATTACK":
            ...
            if st.button("STRIKE",key="inc_attack",use_container_width=True,disabled=not selected_targets):
                try: _inc_combat_attack(run,ch,selected_targets,st.session_state.get(wk),bonus_die=int(spend))
